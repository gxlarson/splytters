...
The unsupervised figures are generated by `demos/visualize_splits.py` and the supervised one by `demos/visualize_supervised_splits.py` — each row is a 2D distribution (unimodal, moons, spirals, rings, ...), each column a splitter. Blue = train, orange = test; in the supervised figure marker shape encodes the class. (`minority_split` shows "failed" on cleanly separated blobs, where every cluster is label-pure by design.)
